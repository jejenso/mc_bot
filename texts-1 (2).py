...
def admin_info_text(user) -> str:
    days_line = f"до {user['access_until']}" if user["access_until"] else "не установлен"
    return (
        f"👤 Профиль игрока\n\n"
        f"ФИ: {user['full_name']}\n"
        f"Ник: {user['mc_nick']}\n"
...
def admin_players_list(users) -> str:
    if not users:
        return "Пока никто не зарегистрирован."
    lines = ["📋 Все игроки:\n"]
    for u in users:
        until = u["access_until"] or "—"
        lines.append(f"• {u['mc_nick']} — {u['status']}, до {until}")
    return "\n".join(lines)
