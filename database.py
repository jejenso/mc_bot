...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        """)
        # миграция для баз, созданных раньше (без этих колонок)
...
async def create_pending_user(user_id: int, full_name: str, mc_nick: str, utc_offset: float | None = None):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            """INSERT INTO users (user_id, full_name, mc_nick, status, created_at, utc_offset)
               VALUES (?, ?, ?, 'pending', ?, ?)
            ...
            (user_id, full_name, mc_nick, datetime.datetime.now().isoformat(), utc_offset),
        )
        await db.commit()
...
async def set_user_status(user_id: int, status: str, access_until: str | None = None):
    async with aiosqlite.connect(DB_PATH) as db:
        if access_until is not None:
            await db.execute(
                "UPDATE users SET status=?, access_until=?, notified_soon=0, notified_hour=0, notified_expired=0 WHERE user_id=?",
...
async def ban_user(user_id: int):
    """Бан: статус banned И дни явно обнуляются (в отличие от set_user_status, тут access_until точно сбрасывается)."""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET status='banned', access_until=NULL, notified_soon=0, notified_hour=0, notified_expired=0 WHERE user_id=?",
            (user_id,),
        )
        await db.commit()
...
async def unban_user(user_id: int):
    """Разбан: статус approved, доступ пока пустой (без оплаты дней нет)."""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET status='approved', access_until=NULL, notified_soon=0, notified_hour=0, notified_expired=0 WHERE user_id=?",
            (user_id,),
        )
        await db.commit()
...
async def reduce_access(user_id: int, days: int):
    user = await get_user(user_id)
    now = datetime.datetime.now()
    if user and user["access_until"]:
        current = datetime.datetime.fromisoformat(user["access_until"])
    ...
    await set_user_status(user_id, status, new_until.isoformat())
    return new_until
...
async def get_all_users():
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute("SELECT * FROM users ORDER BY mc_nick COLLATE NOCASE")
        return await cur.fetchall()


async def get_user_by_nick(mc_nick: str):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
            "SELECT * FROM users WHERE LOWER(mc_nick) = LOWER(?)", (mc_nick,)
        )
        return await cur.fetchone()
